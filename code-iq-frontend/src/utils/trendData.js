...
export function generateTrendData(repoId) {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    const health = Math.max(0, Math.min(10, baseHealth + (dayProgress * 0.5) + randomVariation()));
    const security = Math.max(0, Math.min(10, baseSecurity + (dayProgress * 0.3) + randomVariation()));
    const knowledge = Math.max(0, Math.min(10, baseKnowledge + (dayProgress * 0.7) + randomVariation()));
...
export function generateWeeklyTrendData(repoId) {
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    const health = Math.max(0, Math.min(10, baseHealth + (weekProgress * 1.0) + randomVariation()));
    const security = Math.max(0, Math.min(10, baseSecurity + (weekProgress * 0.8) + randomVariation()));
    const knowledge = Math.max(0, Math.min(10, baseKnowledge + (weekProgress * 1.2) + randomVariation()));
    const overall = (health + security + knowledge) / 3;
...
export function generateHistoricalData(repoId) {
  const events = [
    { date: '2024-12-01', type: 'security_scan', impact: { security: 1.2, health: 0.3 } },
    { date: '2024-12-15', type: 'code_review', impact: { health: 0.8, knowledge: 0.5 } },
    { date: '2025-01-01', type: 'dependency_update', impact: { security: 0.9, health: 0.4 } },
    { date: '2025-01-15', type: 'documentation', impact: { knowledge: 1.1, health: 0.2 } },
    { date: '2025-02-01', type: 'refactoring', impact: { health: 1.0, knowledge: 0.6 } },
  ];
  ...
  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
    ...
    if (event) {
      Object.keys(event.impact).forEach(metric => {
        currentScores[metric] = Math.min(10, currentScores[metric] + event.impact[metric]);
      });
    }
    ...
    Object.keys(currentScores).forEach(metric => {
      ...
      currentScores[metric] = Math.max(0, Math.min(10, currentScores[metric] + variation));
    });
    
    const overall = (currentScores.health + currentScores.security + currentScores.knowledge) / 3;
